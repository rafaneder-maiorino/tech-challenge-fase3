...
DISCIPLINAS = [
    ...
    (
        ...
        "04-monitoracao-performance.md",
        "Monitoração de Performance",
        8,
        8,
    ),
    (
        "Servicos de Monitoracao",
        "05-servicos-monitoracao.md",
...
def main() -> None:
    ...
    linhas.append(
        "> Base de conhecimento consolidada dos PDFs de aula da FIAP Pós "
        "Tech MLET — Fase 3 (Cloud and MLOps)."
    )
    linhas.append("> Aluno: RM 373042 · Data de extração: 2026-07-23")
    linhas.append("")
    linhas.append(
        "Esta pasta reúne a transcrição fiel do conteúdo das 6 disciplinas "
        "da fase, o enunciado do Tech Challenge e o mapa que cruza cada "
        "requisito do TC com as aulas que o fundamentam."
    )
    linhas.append("")
    ...
    linhas.append("|---|---|---|")
    linhas.append(
        "| 00 | [00-INDICE.md](00-INDICE.md) | Este índice + resultado do inventário |"
    )
    for _, arq, rotulo, n, m in DISCIPLINAS:
        linhas.append(f"| {arq[:2]} | [{arq}]({arq}) | {rotulo} ({n} de {m} aulas) |")
    linhas.append(
        "| 07 | [07-tech-challenge-fase3.md](07-tech-challenge-fase3.md) | "
        "Transcrição integral do enunciado do Tech Challenge |"
    )
    linhas.append(
        "| 99 | [99-MAPA-TECH-CHALLENGE.md](99-MAPA-TECH-CHALLENGE.md) | "
    ...
    linhas.append("")
    linhas.append(
        "Percurso recursivo de `~/estudos/fiap/fase3` com `pathlib` "
        "(somente leitura). Todos os 46 PDFs têm texto nativo extraível — "
        "**nenhum exigiu OCR**. Extração com `pdfplumber 0.11.8` "
        "(fallback `pypdf 6.14.2`)."
    )
    linhas.append("")
    linhas.append(
        "| Disciplina (pasta no disco) | PDFs | Extraídos | OCR | Observação |"
    )
    linhas.append("|---|---|---|---|---|")
    rotulos_inv = [
        ...
        (
            ...
            "8",
            "—",
            "`:` literal no nome da pasta no filesystem",
        ),
        (
            "Latência e Performance em Modelos de Dados Não Estruturados",
            "Latência e Performance em Modelos de Dados Não Estruturados",
            ...
            "7",
            "—",
            "**Aula 2 ausente** (não disponibilizada pela FIAP — lacuna conhecida)",
        ),
        (
            "Monitoramento de Performance",
            "Monitoramento de Performance",
            ...
            "8",
            "—",
            'Aula 1 = arquivo "Material Complementar" (capa: Latência vs. Throughput)',
        ),
        (
            "Pipeline de Treino e Deploy Automático",
            "Pipeline de Treino e Deploy Automático",
    ...
    linhas.append("")
    for chave, _arq, rotulo, _n, _m in DISCIPLINAS:
        ...
        for it in itens:
            ...
            linhas.append(
                f"| {it['aula']} | `{it['nome_nfc']}` | {it['paginas']} | {status} |"
            )
    ...
    linhas.append("")
    linhas.append(
        '- **Latência e Performance — Aula 2 "Desafios de Performance em '
        'NLP e Áudio"**: não foi disponibilizada pela FIAP no portal '
        "(confirmado pelo aluno). Não é falha de extração. Como o Tech "
        "Challenge é um classificador de texto (NLP), esta é a lacuna de "
        "maior impacto — a cobertura compensatória está mapeada em "
